...
import type { Eip1193Provider, RpcRequestMap, TypedEip1193Provider } from "../types.js"
...
export class Eip1193Account {
  ...
  constructor(
    ...
    readonly aztecNode: Pick<
      ...
      "getTxEffect" | "getTxReceipt" | "getUnencryptedLogs" | "getProvenBlockNumber"
    >,
  ) {
  ...
  async simulateTransaction(txRequest: Pick<TransactionRequest, "calls">): Promise<string[]> {
    return await this.provider.request({
      method: "aztec_call",
      params: [
        {
          from: this.address.toString(),
          calls: await Promise.all(txRequest.calls.map((x) => encodeFunctionCall(x))),
        },
      ],
    })
  }
    ...
    async request(params) {
      ...
      const methodMap: {
        ...
        aztec_sendTransaction: async (request) => {
          ...
          const calls = await Promise.all(request.calls.map((x) => decodeFunctionCall(account, x)))
          ...
          const tx = await account.proveTx(txRequest, simulatedTx.privateExecutionResult)
          const txHash = await new SentTx(account, account.sendTx(tx.toTx())).getTxHash()
        ...
        aztec_call: async (request) => {
          ...
          if (simulatedTx) {
            for (const [call, callIndex, resultIndex] of indexedCalls) {
              ...
              results[callIndex] = await serde.FrArray.serialize(rawReturnValues ?? [])
            }
          }
          return results
        },
