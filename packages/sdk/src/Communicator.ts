...
export class Communicator {
  ...
  postRequestAndWaitForResponse = async <M extends Message>(request: Message): Promise<M> => {
    ...
    let type: WindowType = this.iframe.rpcCalls.includes(request.data.method) ? "iframe" : "popup"
  ...
  waitForPopupLoaded = async (): Promise<Window> => {
    ...
    if (!this.popup.window && this.popup.fallbackOpenPopup) {
      ...
      this.popup.window = await this.popup.fallbackOpenPopup(() => openPopup(this.popup.url))
    }
    if (!this.popup.window) {
      throw new Error("Failed to open popup: failed to load")
    }

    this.onMessage<ConfigMessage>(({ event }) => event === "PopupUnload", "popup")
    ...
    const pingInterval: any = setInterval(() => {
      ...
      this.popup.window.postMessage({ event: "PopupLoadedRequest" }, this.popup.url.origin)
    }, 100)
    try {
  ...
  private async waitForIframeLoaded(): Promise<Window> {
    ...
    const pingInterval = setInterval(() => {
      ...
      // Send a ping message
      iframeWindow.postMessage({ event: "IFrameLoadedRequest" }, this.iframe.url.origin)
    }, 100)
