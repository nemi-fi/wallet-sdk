...
export class ContractBase<T extends AztecContract> {
  ...
  protected constructor(
    ...
    this.methods = artifact.functions.reduce((acc, f) => {
      acc[f.name as keyof T["methods"]] = Object.assign(
        (...argsAndOptions: any[]) => {
          ...
          return new ContractFunctionInteraction(this.address, this.account, f, args, options)
        },
        {
          get selector() {
...
export namespace Contract {
  export type Infer<T extends { at: (...args: any[]) => any }> = Awaited<ReturnType<T["at"]>>
}
...
class ContractFunctionInteraction {
  ...
  async simulate() {
    ...
    return decodeFromAbi(this.functionAbi.returnTypes, await serde.FrArray.deserialize(result))
  }
